<form wire:submit="submitForm" class=" flex justify-center">
                ...
                <tbody>
                    @if ($regulations)
                        @foreach ($regulations as $key => $value)
                            <tr wire:click="regulationSelected({{$value->id}})" class="bg-white border-b hover:bg-gray-50 cursor-pointer">
                                <th scope="row"
    ...
    <div class="w-4/12 mx-2 rounded border p-2">
            ...
            <input wire:model.live="searchTerm"  class="block w-full p-2.5 ps-10 text-sm text-gray-500 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Пронађи пропис ...."  />
        </div>
        <div>
            <div>
