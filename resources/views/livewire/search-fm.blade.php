<div class="relative">
    <div class="flex items-center w-full">
        ...
        <!-- Input za pretragu -->
        <input type="search" wire:model.live="searchTerm" id="search-dropdown" class="block w-full p-2.5 text-sm text-gray-900 bg-gray-50 border-l-0  border border-gray-300 rounded-r-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white dark:border-gray-600 dark:focus:ring-blue-700 dark:focus:border-blue-700" autocomplete="off" placeholder="Претрага формацијског места ..." />
    ...
    <ul class="absolute z-50 w-full bg-white text-gray-900 mx-1 text-left list-inside dark:text-gray-400 mt-1 overflow-hidden">
        @if ($results && $results->count() > 0)
            @foreach ($results as $result)
                <li wire:click="$dispatch('fmSelected', [{{$result->id}}])" class="border rounded-sm cursor-pointer p-2">
                    {{ $result->name }}
                </li>
            @endforeach
